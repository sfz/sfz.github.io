...
[numbered LFOs][1], rather than the [dedicated pitch, volume and filter LFOs][2]
...
and add vibrato a fraction of a second later. This is where modulating the [LFO delay parameter][3] becomes useful, and possibly [LFO fade][4] as well. Delay seems like enough for most wind
...
[waveform][5] can be used here, and the modulation depth controlled by MIDI CC, so when that's
...
[starting phase][6] will also work:
...
pseudo-randomize the starting phase using a non-random [CC, such as velocity][7]
...
set so the wave starts at the top, and the note [tuned][8] down by the vibrato depth amount.
...
can combine the LFO with a pitch [envelope][9]. Here is an example with just delay:
...
regions and use [loccN/hiccN][10] to select between ones with different styles of vibrato.
...
modulate [filter cutoffs][11], which is commonly used in synthesizers to create
...
[1]:        ../modulations/lfo.md
[2]:        ../opcodes/amplfo_depth.md
[3]:        ../opcodes/lfoN_delay.md
[4]:        ../opcodes/lfoN_fade.md
[5]:        ../opcodes/lfoN_wave.md
[6]:        ../opcodes/lfoN_phase.md
[7]:        ../extensions/midi_ccs.md
[8]:        ../opcodes/tune.md
[9]:        ../modulations/envelope_generators.md
[10]:       ../opcodes/loccN.md
[11]:       ../opcodes/cutoff.md
