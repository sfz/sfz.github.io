...
The three EQ bands' frequency, bandwidth and gain work similarly, but also
add velocity tracking. The EQ bandwidth and center frequency also have non-zero defaults,
...
EQ frequency and gain (but not bandwidth) can additionally be modulated by
velocity. For example, if we want to make a sound brighter when the velocity is
higher, we might use something like this:
...
The xfin/xfout CCs are also a way to fade sounds in and out using MIDI CC. An
example of one note with two dynamic layers being crossfaded:
...
modulation sources, and the xf_cccurve, xf_keycurve and xf_velcurve give the choice
of two curves for each of these modulations.
...
"gain" in the name, while the others are called amp_random, amp_keytrack and
amp_veltrack.
